...
import {
  ...
  Send, Award, Download, X, Sun, Moon,
} from 'lucide-react';
import { analyzeArt, ArtAnalysisResult, FinalPaletteColor, chatWithBelle, ChatMessage } from './services/geminiService';

// ─── THEME FACTORIES ─────────────────────────────────────────
const DARK = {
  bg: '#121417', surface: '#1E2126', surfaceAlpha: 'rgba(30,33,38,0.75)',
  accent: '#D4AF37', accentHover: '#E5C04B',
  text: '#F3F4F6', textSec: '#9CA3AF',
  glass: 'rgba(0,0,0,0.30)', glassBorder: 'rgba(255,255,255,0.10)',
  userBubble: '#2563eb', sidebar: 'rgba(18,20,23,0.92)',
};
const LIGHT = {
  bg: '#FBF9F6', surface: '#FFFFFF', surfaceAlpha: 'rgba(255,255,255,0.80)',
  accent: '#B8860B', accentHover: '#996F09',
  text: '#1F2937', textSec: '#6B7280',
  glass: 'rgba(255,255,255,0.60)', glassBorder: 'rgba(0,0,0,0.05)',
  userBubble: '#2563eb', sidebar: 'rgba(255,255,255,0.92)',
};
...
function buildTheme(mode: 'dark' | 'light') {
  ...
  return createTheme({
    ...
    components: {
      ...
      MuiButton: { styleOverrides: { root: { textTransform: 'none' as const, fontWeight: 600, letterSpacing: '0.01em' } } },
      MuiTab: { styleOverrides: { root: { textTransform: 'none' as const, fontWeight: 600, fontSize: '0.82rem' } } },
    },
  });
}
...
// ─── THEME TOGGLE ────────────────────────────────────────────
const ThemeToggle: React.FC<{ mode: 'dark' | 'light'; onToggle: () => void }> = ({ mode, onToggle }) => (
  <motion.button
...
const ArtLoader = () => (
  <motion.div
    ...
    style={{
      ...
      background: 'var(--loader-overlay)', backdropFilter: 'blur(12px)',
  ...
  >
    <Box sx={{ position: 'relative', mb: 4 }}>
      <motion.div
        ...
        style={{ width: 72, height: 72, background: 'linear-gradient(135deg, var(--accent), #6366f1)', filter: 'blur(10px)', opacity: 0.5, borderRadius: '30% 70% 70% 30%' }}
      />
    ...
    <Typography sx={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', color: 'var(--text-primary)', fontSize: '1.15rem', opacity: 0.85 }}>
    ...
    >
      ...
      >
        ...
        <Box className="backface-hidden" sx={{
          ...
          p: 2.5, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1.5,
          '&:hover': { borderColor: 'var(--accent)' }, transition: 'all 0.25s',
        }}>
          <Box sx={{ position: 'relative' }}>
            ...
            />
            {unfitInfo && (
              <Box sx={{ position: 'absolute', top: -5, right: -5, background: '#ef4444', borderRadius: '50%', p: 0.4, display: 'flex' }}>
                <AlertTriangle size={11} color="white" />
              </Box>
            )}
          </Box>
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="body2" sx={{ fontWeight: 600, textTransform: 'capitalize', mb: 0.3, fontSize: '0.82rem' }}>{color.name}</Typography>
            <Typography variant="caption" sx={{ fontFamily: '"DM Sans", monospace', color: 'var(--text-secondary)', background: 'var(--chip-bg)', px: 1, py: 0.3, borderRadius: 1, display: 'block', fontSize: '0.7rem' }}>
              {color.hex}
            </Typography>
          </Box>
          <Typography variant="caption" sx={{ color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.15em', fontSize: '0.55rem', position: 'absolute', bottom: 10 }}>
        ...
        <Box className="backface-hidden" sx={{
          ...
          backdropFilter: 'blur(20px)', borderRadius: 3, p: 2.5,
          transform: 'rotateY(180deg)',
          display: 'flex', flexDirection: 'column', overflow: 'hidden',
        }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5, pb: 1.5, borderBottom: '1px solid var(--border-subtle)' }}>
            <Box sx={{ width: 12, height: 12, borderRadius: '50%', bgcolor: color.hex, flexShrink: 0, border: '1px solid var(--border-glass)' }} />
            <Typography variant="caption" sx={{ fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', fontSize: '0.62rem' }}>{color.name}</Typography>
          </Box>
          {unfitInfo && (
            <Box sx={{ background: 'var(--error-bg)', border: '1px solid var(--error-border)', borderRadius: 2, p: 1.2, mb: 1.5, display: 'flex', gap: 1 }}>
              <AlertTriangle size={11} style={{ color: 'var(--error-text)', flexShrink: 0, marginTop: 1 }} />
              <Typography variant="caption" sx={{ color: 'var(--error-text)', fontSize: '0.62rem' }}>{unfitInfo.reason}</Typography>
            </Box>
          )}
          <Typography variant="caption" sx={{ color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.12em', fontSize: '0.55rem', fontWeight: 700, mb: 1, display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Droplet size={10} /> Công thức
          </Typography>
          <Typography variant="caption" sx={{ color: 'var(--text-secondary)', lineHeight: 1.7, fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', fontSize: '0.72rem', overflow: 'auto', flex: 1 }}>
            {color.mixingGuide}
          </Typography>
        </Box>
      </motion.div>
...
const PaletteCard = ({ image, colors, critique, onClose }: { image: string; colors: any[]; critique: string; onClose: () => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ position: 'fixed', inset: 0, zIndex: 1300, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.80)', backdropFilter: 'blur(12px)', padding: 16 }}>
      <motion.div initial={{ scale: 0.85, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.85, y: 30 }}
        transition={{ type: 'spring', stiffness: 260, damping: 24 }}
        style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
        <IconButton onClick={onClose} sx={{ position: 'absolute', top: -48, right: 0, color: 'rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.08)', '&:hover': { background: 'rgba(255,255,255,0.15)' } }}>
          <X size={20} />
        </IconButton>
        <Box ref={cardRef} sx={{ width: 420, p: 4, background: 'var(--palette-card-bg)', border: '1px solid var(--border-glass)', position: 'relative', overflow: 'hidden' }}>
          <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, var(--accent), var(--accent-hover), var(--accent))` }} />
          <Typography variant="h6" sx={{ textAlign: 'center', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--accent)', fontFamily: '"Cormorant Garamond", serif', mb: 3, fontWeight: 700, fontSize: '1.1rem' }}>
            Belleza Lab
          </Typography>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, px: 1 }}>
            {colors.slice(0, 5).map((c, i) => (
              <Box key={i} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.75 }}>
                <Box sx={{ width: 48, height: 48, borderRadius: 2, bgcolor: c.hex, border: '1px solid var(--border-glass)', boxShadow: `0 4px 12px ${c.hex}33` }} />
                <Typography variant="caption" sx={{ fontFamily: 'monospace', fontSize: '0.55rem', color: 'var(--text-secondary)', textTransform: 'uppercase' }}>{c.hex}</Typography>
              </Box>
            ))}
          </Box>
          <Box sx={{ borderTop: '1px solid var(--border-subtle)', pt: 2, textAlign: 'center' }}>
            <Typography variant="body2" sx={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', color: 'var(--text-secondary)', lineHeight: 1.7, fontSize: '0.78rem', px: 2 }}>
              "{critique}"
            </Typography>
          </Box>
          <Box sx={{ mt: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            <Box sx={{ width: 3, height: 3, borderRadius: '50%', bgcolor: 'var(--accent)' }} />
            <Typography variant="caption" sx={{ color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.15em', fontSize: '0.52rem' }}>
              Analyzed by Belle · Art Intelligence
            </Typography>
            <Box sx={{ width: 3, height: 3, borderRadius: '50%', bgcolor: 'var(--accent)' }} />
          </Box>
        </Box>
        <AccentButton onClick={downloadCard} disabled={isDownloading} startIcon={<Download size={16} />} sx={{ mt: 3 }}>
...
export default function App() {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          if (isSketch && analysis.determined_mood) {
            setChatHistory([{ role: 'model', text: `Chào Tuấn! Bức phác thảo này gợi cảm giác ${analysis.determined_mood}. Belle đã chọn bảng màu phong cách ${analysis.suggested_style}. Bạn thấy sao?` }]);
          }
  ...
  const handleSendMessage = async (text: string, isFinalReview = false) => {
    ...
    try {
      ...
      console.error(err); setChatHistory(prev => [...prev, { role: 'model', text: 'Xin lỗi, Belle đang gặp sự cố. Bạn thử lại sau nhé!' }]);
    ...
    <ThemeProvider theme={theme}>
      ...
      <AnimatePresence>
        {imageSrc && (
          <motion.div key="img-bg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.5 }}
            style={{ position: 'fixed', inset: 0, zIndex: 0, backgroundImage: `url(${imageSrc})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(120px) saturate(0.35)', opacity: isDark ? 0.06 : 0.04 }}
          />
        )}
      </AnimatePresence>
      ...
      <Box sx={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, height: '100vh', overflow: 'hidden' }}>
        ...
        <Box component="main" sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: { xs: 3, md: 6 }, overflowY: 'auto', height: '100%' }}>
          <Box sx={{ width: '100%', maxWidth: 680, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            {/* Header */}
            <motion.div initial={{ opacity: 0, y: -25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} style={{ textAlign: 'center', marginBottom: 48 }}>
              <Typography variant="h1" className="animated-logo" sx={{ fontSize: { xs: '2.8rem', md: '4.2rem' }, mb: 1, lineHeight: 1, letterSpacing: '-0.02em' }}>
                Belleza Lab
              </Typography>
              <Typography variant="body1" sx={{ color: 'text.secondary', maxWidth: 400, mx: 'auto', lineHeight: 1.7, fontSize: '0.92rem', letterSpacing: '0.01em' }}>
                Mã hóa nghệ thuật · Giải phóng bảng màu
              </Typography>
              <Typography variant="body2" sx={{ color: 'var(--accent)', fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', mt: 0.5, opacity: 0.6, fontSize: '0.85rem' }}>
            ...
            <AnimatePresence mode="wait">
              {!imageSrc ? (
                <motion.div key="upload" initial={{ opacity: 0, scale: 0.96, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.93 }} transition={{ duration: 0.4 }} style={{ width: '100%' }}>
                  ...
                  <GlassCard sx={{ p: 2.5, mb: 3, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2, alignItems: 'center', justifyContent: 'space-between' }}>
                    <Box component="label" sx={{ display: 'flex', alignItems: 'center', gap: 1.5, cursor: 'pointer' }}>
                      ...
                      <Box sx={{ width: 38, height: 20, borderRadius: 3, position: 'relative', background: isSketch ? 'var(--accent)' : 'var(--chip-bg)', border: `1px solid ${isSketch ? 'var(--accent)' : 'var(--chip-border)'}`, transition: 'all 0.3s', flexShrink: 0 }}>
                        <Box sx={{ position: 'absolute', top: 2, width: 14, height: 14, borderRadius: '50%', background: isDark ? '#fff' : (isSketch ? '#fff' : '#9CA3AF'), left: isSketch ? 'calc(100% - 16px)' : 2, transition: 'left 0.3s cubic-bezier(0.34,1.56,0.64,1)' }} />
                      </Box>
                          ...
                          <Box sx={{ display: 'flex', gap: 1.5 }}>
                            {[
                              ...
                              <Box key={idx} component="select" value={sel.val} onChange={(e: any) => sel.setter(e.target.value)}
                                sx={{ background: 'var(--select-bg)', border: '1px solid var(--select-border)', borderRadius: 2, px: 1.5, py: 0.8, color: 'var(--text-primary)', fontSize: '0.78rem', cursor: 'pointer', outline: 'none', fontFamily: '"DM Sans", sans-serif' }}>
                  ...
                  >
                    ...
                    {/* Decorative corners */}
                    {[{ top: 14, left: 14, bt: true, bl: true }, { top: 14, right: 14, bt: true, br: true }, { bottom: 14, left: 14, bb: true, bl: true }, { bottom: 14, right: 14, bb: true, br: true }].map((c: any, i) => (
                      <Box key={i} sx={{ position: 'absolute', width: 18, height: 18,
                        ...(c.top !== undefined && { top: c.top }), ...(c.bottom !== undefined && { bottom: c.bottom }),
                    ...
                    <motion.div animate={isDragging ? { scale: [1, 1.15, 1] } : {}} transition={{ duration: 0.4 }}>
                      <Box sx={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--accent-subtle)', border: `1px solid ${isDragging ? accentCol : borderCol}`, display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 3, color: isDragging ? accentCol : 'var(--text-secondary)', transition: 'all 0.3s' }}>
                        <UploadCloud size={32} />
                      </Box>
                    </motion.div>
                ...
                <motion.div key="preview" initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} style={{ width: '100%' }}>
                  <Box sx={{ position: 'relative', borderRadius: 3, overflow: 'hidden', boxShadow: isDark ? '0 24px 60px -12px rgba(0,0,0,0.6)' : '0 16px 48px -12px rgba(0,0,0,0.12)', border: `1px solid ${borderCol}` }}>
                  ...
                  <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Box sx={{ mb: 3, background: 'var(--chip-bg)', borderRadius: 2.5, p: 0.5, border: `1px solid ${borderSubtle}` }}>
                      <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)} variant="fullWidth"
                        ...
                        sx={{
                          minHeight: 38,
                          '& .MuiTab-root': { minHeight: 38, borderRadius: 2, color: 'text.secondary', transition: 'all 0.25s', fontSize: '0.82rem' },
                      ...
                      <AnimatePresence mode="wait">
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                              <Box>
                                ...
                                <GlassCard sx={{ p: 3, position: 'relative', overflow: 'hidden' }}>
                                  ...
                                  <Typography variant="h6" sx={{ mb: 1, fontSize: '1.05rem' }}>{analysisResult.medium}</Typography>
                                  <Typography variant="body2" sx={{ color: 'text.secondary', fontStyle: 'italic', lineHeight: 1.7, fontSize: '0.85rem' }}>{analysisResult.mediumDescription}</Typography>
                                </GlassCard>
                              </Box>
                              {isSketch && analysisResult.placement_guide && (
                                <Box>
                                  ...
                                  <GlassCard sx={{ p: 3 }}>
                                    <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.8, fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', whiteSpace: 'pre-wrap' }}>{analysisResult.placement_guide}</Typography>
                                  </GlassCard>
                                </Box>
                              )}
                              <Box>
                                ...
                                <motion.div variants={containerVariants} initial="hidden" animate="show" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                                  {analysisResult.current_colors.map((color, i) => {
                                    const unfit = analysisResult.issue?.unfit_colors?.find(u => u.hex.toLowerCase() === color.hex.toLowerCase());
                          ...
                          <motion.div key="optimized" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                              <Box>
                                ...
                                <GlassCard sx={{ p: 3 }}>
                                  <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.8, fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', mb: 2.5, fontSize: '0.88rem' }}>{analysisResult.issue?.critique}</Typography>
                                  <Typography variant="overline" sx={{ fontSize: '0.55rem', letterSpacing: '0.12em', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 0.5, mb: 1.5 }}>
                                    <CheckCircle size={11} /> Thay thế đề xuất
                                  </Typography>
                                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                    {analysisResult.suggestions?.replacement_colors?.map((color, i) => (
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                          ...
                                          <Box sx={{ flex: 1 }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                              <Typography variant="body2" sx={{ fontWeight: 700, fontSize: '0.85rem' }}>{color.name}</Typography>
                                              <Typography variant="caption" sx={{ fontFamily: 'monospace', color: 'text.secondary', fontSize: '0.62rem' }}>{color.hex}</Typography>
                                            </Box>
                                            <Typography variant="caption" sx={{ color: 'text.secondary', fontStyle: 'italic', fontSize: '0.75rem' }}>{color.why}</Typography>
                                          </Box>
                                        </Box>
                                      </motion.div>
                                    ))}
                              ...
                              <Box>
                                ...
                                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mb: 2 }}>
                                  {analysisResult.suggestions.final_palette.map((color, i) => {
                                    const isSelected = selectedColor?.hex === color.hex;
                                    const tc = getTextColor(color.hex);
                                    return (
                                      <motion.div key={i} whileHover={{ scale: 1.04, y: -4 }} whileTap={{ scale: 0.96 }}
                                        onClick={() => setSelectedColor(color)}
                                        style={{ borderRadius: 14, padding: '14px', aspectRatio: '1', cursor: 'pointer', backgroundColor: color.hex, position: 'relative', overflow: 'hidden', boxShadow: isSelected ? `0 0 0 2px ${tc === '#ffffff' ? 'rgba(255,255,255,0.8)' : 'rgba(0,0,0,0.6)'}, 0 0 0 4px #6366f1` : `0 6px 20px ${color.hex}33` }}>
                                        <Typography variant="caption" style={{ color: tc, opacity: 0.6, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', fontSize: '0.55rem', display: 'block' }}>{color.role}</Typography>
                                        <Box style={{ position: 'absolute', bottom: 10, left: 10 }}>
                                          <Typography variant="body2" style={{ color: tc, fontWeight: 700, fontSize: '0.82rem' }}>{color.name}</Typography>
                                          <Typography variant="caption" style={{ color: tc, opacity: 0.6, fontFamily: 'monospace', fontSize: '0.58rem' }}>{color.hex}</Typography>
                                        </Box>
                                        {isSelected && <Box style={{ position: 'absolute', top: 6, right: 6, background: 'var(--card-badge-bg)', borderRadius: '50%', padding: 2, display: 'flex' }}><CheckCircle size={13} color={tc} /></Box>}
                                      </motion.div>
                                    );
                                  })}
                                </Box>
                                <AnimatePresence mode="wait">
                                  {selectedColor && (
                                    <motion.div key={selectedColor.hex} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.22 }}>
                                      <GlassCard sx={{ p: 3, position: 'relative', overflow: 'hidden' }}>
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5, pl: 1 }}>
                                          ...
                                          <Typography variant="body2" sx={{ fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', fontSize: '0.78rem' }}>
                                            Công thức: {selectedColor.name}
                                          </Typography>
                                        </Box>
                                        <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.8, fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', pl: 1, fontSize: '0.88rem' }}>
                        ...
                        {activeTab === 'belle' && (
                          <motion.div key="belle" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}
                            style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', borderRadius: 16, border: `1px solid ${borderCol}`, overflow: 'hidden', background: 'var(--glass-bg)' }}>
                            ...
                            <Box sx={{ p: 2.5, borderBottom: `1px solid ${borderSubtle}`, background: 'var(--chip-bg)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                <Box sx={{ position: 'relative' }}>
                                  <Avatar sx={{ width: 44, height: 44, borderRadius: 2, background: `linear-gradient(135deg, var(--accent), #8B6914)`, fontSize: '1.2rem', fontFamily: '"Cormorant Garamond", serif', fontWeight: 700, boxShadow: '0 3px 12px var(--accent-glow)' }}>B</Avatar>
                            ...
                            <Box sx={{ flex: 1, overflowY: 'auto', p: 2.5, display: 'flex', flexDirection: 'column', gap: 2 }}>
                              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
                                <Box sx={{ maxWidth: '88%', p: 2, background: 'var(--bg-surface-alpha)', border: `1px solid ${borderCol}`, borderRadius: '18px 18px 18px 4px' }}>
                                  <Typography variant="body2" sx={{ lineHeight: 1.7, fontFamily: '"Cormorant Garamond", serif', fontStyle: 'italic', fontSize: '0.88rem' }} className="typewriter-cursor">
                              ...
                              {chatHistory.map((msg, idx) => (
                                <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                                  ...
                                  <Box sx={{
                                    maxWidth: '88%', p: 2,
                                    ...(msg.role === 'user'
                                      ? { background: 'var(--user-bubble)', color: '#fff', borderRadius: '18px 18px 4px 18px', boxShadow: '0 3px 12px var(--user-bubble-shadow)' }
                                      : { background: 'var(--bg-surface-alpha)', border: `1px solid ${borderCol}`, borderRadius: '18px 18px 18px 4px' }),
                                  }}>
                                    <Typography variant="body2" sx={{ lineHeight: 1.7, fontFamily: msg.role === 'model' ? '"Cormorant Garamond", serif' : 'inherit', fontStyle: msg.role === 'model' ? 'italic' : 'normal', whiteSpace: 'pre-wrap', fontSize: '0.88rem' }}>
                                      {msg.text}
                                    </Typography>
                                  </Box>
                                </motion.div>
                              ...
                              {isChatting && (
                                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
                                  <Box sx={{ p: 2, background: 'var(--bg-surface-alpha)', border: `1px solid ${borderCol}`, borderRadius: '18px 18px 18px 4px', display: 'inline-flex', gap: 0.6, alignItems: 'center' }}>
                                    {[0, 1, 2].map(i => (
                            ...
                            <Box sx={{ p: 2.5, borderTop: `1px solid ${borderSubtle}`, background: 'var(--chip-bg)', flexShrink: 0 }}>
                                ...
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.8, mb: 2 }}>
                                  {[{ t: 'Bố cục đã ổn chưa?', i: '🎨' }, { t: 'Gợi ý cách vẽ mây.', i: '💡' }, { t: 'Hợp với không gian nào?', i: '🏠' }].map((btn, i) => (
                                    <Chip key={i} label={`${btn.i} ${btn.t}`} size="small" onClick={() => handleSendMessage(btn.t)}
                                      sx={{ cursor: 'pointer', fontSize: '0.68rem', background: 'var(--chip-bg)', border: `1px solid var(--chip-border)`, '&:hover': { borderColor: accentCol, color: accentCol } }} />
                                  ))}
                                </Box>
                              )}
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
                                <AccentButton size="small" startIcon={<Award size={14} />} onClick={() => handleSendMessage('', true)} disabled={isChatting} sx={{ px: 2, py: 0.8, fontSize: '0.73rem' }}>
                                  Final Review
                                </AccentButton>
                                {finalReviewText && (
                                  <GhostButton size="small" startIcon={<ImageIcon size={14} />} onClick={() => setShowPaletteCard(true)} sx={{ px: 2, py: 0.8, fontSize: '0.73rem' }}>
                                    Xem Thẻ Màu
                                  </GhostButton>
                                )}
                              </Box>
                              <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                <TextField fullWidth size="small" value={chatInput}
                                  ...
                                  sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2.5, background: 'var(--glass-bg)', '& fieldset': { borderColor: borderCol }, '&:hover fieldset': { borderColor: accentCol }, '&.Mui-focused fieldset': { borderColor: accentCol }, '& input': { color: 'var(--text-primary)', fontSize: '0.82rem', fontFamily: '"DM Sans", sans-serif' } } }}
                                />
                                <motion.div whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}>
                                  <IconButton onClick={() => handleSendMessage(chatInput)} disabled={!chatInput.trim() || isChatting}
                                    sx={{ width: 40, height: 40, background: chatInput.trim() ? 'var(--accent)' : 'var(--chip-bg)', color: chatInput.trim() ? '#fff' : 'text.secondary', transition: 'all 0.2s', '&:hover': { background: 'var(--accent-hover)' }, '&:disabled': { opacity: 0.4 } }}>
                                    <Send size={17} />
                                  </IconButton>
                                </motion.div>
                              </Box>
      ...
      <AnimatePresence>
        {showPaletteCard && analysisResult && imageSrc && (
          <PaletteCard image={imageSrc} colors={analysisResult.current_colors} critique={finalReviewText || 'Một tác phẩm tuyệt vời.'} onClose={() => setShowPaletteCard(false)} />
        )}
      </AnimatePresence>
    </ThemeProvider>
  );
